...
#               2023  Modified by: Rodrigo César <rcesarcs@hotmail.com>
#
...
class HallFilamentWidthSensor:
    def __init__(self, config):
        ...
        self.dia2=config.getfloat('Cal_dia2', 1.75)
        self.dia3=config.getfloat('Cal_dia3', 2.0)
        self.rawdia1=config.getint('Raw_dia1', 9500)
        self.rawdia2=config.getint('Raw_dia2', 10500)
        self.rawdia3=config.getint('Raw_dia3', 11000)
        ...
        self.gcode.register_mux_command('QUERY_FILAMENT_WIDTH', "SENSOR", self.name, self.cmd_M407)
        self.gcode.register_mux_command('RESET_FILAMENT_WIDTH_SENSOR', "SENSOR", self.name,
                                    self.cmd_ClearFilamentArray)
        self.gcode.register_mux_command('DISABLE_FILAMENT_WIDTH_SENSOR', "SENSOR", self.name,
                                    self.cmd_M406)
        self.gcode.register_mux_command('ENABLE_FILAMENT_WIDTH_SENSOR', "SENSOR", self.name,
                                    self.cmd_M405)
        self.gcode.register_mux_command('QUERY_RAW_FILAMENT_WIDTH', "SENSOR", self.name,
                                    self.cmd_Get_Raw_Values)
        self.gcode.register_mux_command('ENABLE_FILAMENT_WIDTH_LOG', "SENSOR", self.name,
                                    self.cmd_log_enable)
        self.gcode.register_mux_command('DISABLE_FILAMENT_WIDTH_LOG', "SENSOR", self.name,
    ...
    def handle_ready(self):
        # calculate calibration curve coeficients
        x = [self.rawdia1,self.rawdia2,self.rawdia3]
        y = [self.dia1,self.dia2,self.dia3]
        degree = len(x)-1
        self.coef = np.polyfit(x,y,len(x)-1)
    ...
    def adc2_callback(self, read_time, read_value):
        ...
        power = 0
        logging.info("RAW")
        logging.info(self.lastFilamentWidthReading+self.lastFilamentWidthReading2)
        for c in self.coef:
            diameter_new = diameter_new + c*pow((self.lastFilamentWidthReading+self.lastFilamentWidthReading2),power)
            power = power + 1
    ...
    def cmd_ClearFilamentArray(self, gcmd):
        self.filament_array = []
        gcmd.respond_info("Filament Sensor %s: measurements cleared!" % (self.name))
        # Set extrude multiplier to 100%
        self.gcode.run_script_from_command("M221 S100")

    def cmd_M405(self, gcmd):
        response = "Filament Sensor %s: Turned On" % (self.name)
        if self.is_active:
            response = "Filament Sensor %s: is already On" % (self.name)
        else:
            self.is_active = True
            # Start extrude factor update timer
            self.reactor.update_timer(self.extrude_factor_update_timer,
    ...
    def cmd_M406(self, gcmd):
        response = "Filament Sensor %s: Turned Off" % (self.name)
        if not self.is_active:
            response = "Filament Sensor %s: is already Off" % (self.name)
        else:
    ...
    def get_status(self, eventtime):
        return {'Diameter': round(self.diameter,3),
